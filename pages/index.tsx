...
export default function Home() {
  ...
  useEffect(() => {
    ...
    myFetch(`http://localhost:3000/api/stocks/trending/chart`,{
        ...
        .then((data) => {
          ...
          setChartOptions(() => {
            var chartList = []
            chartList.push(data.trending.data.map((item) => {
  ...
  if (dataLoaded) {
    ...
    chartCarouselList = chartOptions.map((chartDetails) => {
      return (
          <Chart  key={i++} options={chartDetails} />
      )
    });
  ...
  return (
    <>
      {dataLoaded && (
        ...
        </div>{stockNews.length !== 0 || <h1 className={classes.loadingMessage}>Loading News...</h1>}
                {stockNews}
          <div className={classes.chart}>
            {/* <Chart options={chartOptions} size={{ width: 500}} /> */}
          </div>
        </>
      )}
      {/* <StockChart/> */}
