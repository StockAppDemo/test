...
export default authenticated(async function getStocks(
    ...
    if(q.options === 'new'){
      ...
      if (user.stocks.length > 10){
          ...
          res.status(400).send({message:"Free Api only supports 10 stocks at a time"})
          return
      }
    ...
    console.log("Running put email,stocks,body",email,stocks,req.body)
    var stocksJson
        stocksJson=stocks
    const update = {
        "$set": {
    ...
    res.send({message:"we only support GET"})
  }
