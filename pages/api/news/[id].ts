...
export default authenticated(async function getStockDetails(req, res) {
  var id = req.url.split('/')[3];
    console.log(0)
  const detailsPromise = fetch(`https://www.alphavantage.co/query?function=OVERVIEW&symbol=${id}&apikey=${process.env.ALPHAVANTAGE_API}`)
  .then(response => response.json())
  var newsData = await fetch(`https://api.marketaux.com/v1/news/all?symbols=${id}&api_token=${process.env.MARKETAUX_TOKEN}&filter_entities=true&group_similar=true&language=en&min_match_score=50`).then(response => response.json());
